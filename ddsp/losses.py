...
class VAELoss(MultiLoss):
  ...
  def __init__(self,
               ...
               logmag_weight=0.0,
               loudness_weight=0.0,
               cyclic_annealing=True,
               cycle_iterations=3000,
               name='vae_loss'):
    """Constructor.
    """
    super().__init__(name=name)
    ...
    if self.cyclic_annealing:
      frange_cycle = self.frange_cycle_linear(cycle_iterations * 4, n_cycle=4)
      self.beta_cycle = tf.Variable(frange_cycle, trainable=False)
      self.beta_i = tf.Variable(0, trainable=False)
  ...
  def select_beta(self):
    if self.cyclic_annealing:
      ...
      self.beta_i.assign((self.beta_i + 1) % len(self.beta_cycle))
    else:
      beta = 1
    return beta
